...
- **The store is fixed.** Every request goes to `SHOPIFY_STORE_DOMAIN`. No tool accepts a store or token argument. Invalid configuration names the variable that failed; fix it and restart the server.
- **Authentication takes one of two paths.** With `SHOPIFY_CLIENT_ID` and `SHOPIFY_CLIENT_SECRET` of a Dev Dashboard app, the server runs the `client_credentials` grant itself against `https://{store}.myshopify.com/admin/oauth/access_token`: the token lives 24 hours, is cached in memory only, is re-minted `SHOPIFY_TOKEN_LEEWAY_SECONDS` (default 300) before it expires, and parallel tool calls share a single exchange. This grant works only while the app and the store belong to the same Shopify organization. A ready-made `SHOPIFY_ACCESS_TOKEN` — the legacy path for an admin-created custom app issued before 2026-01-01 — is sent as-is and never refreshed; when both are set, the ready-made token wins. With neither, the server still starts and every tool call returns an error naming both options.
...
- **Access is controlled by app scopes.** `ACCESS_DENIED` means the app is missing a scope, not that the token is invalid: add the scope to the app, and a `client_credentials` token picks it up at the next exchange, while a ready-made `SHOPIFY_ACCESS_TOKEN` has to be replaced by hand. A 401 points at the credentials themselves — `SHOPIFY_CLIENT_ID` / `SHOPIFY_CLIENT_SECRET`, or `SHOPIFY_ACCESS_TOKEN`; a minted token that draws a 401 is dropped and re-minted once before the error surfaces, a ready-made one is not. `shop_not_permitted` from the token exchange means the app and the store sit in different Shopify organizations, which no retry and no re-issued secret fixes. A 404 at the API layer usually points to `SHOPIFY_STORE_DOMAIN`. A missing entity is `data: null`, not a transport error.
