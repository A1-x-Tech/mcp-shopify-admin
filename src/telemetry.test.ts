...
test("opt-out sends nothing", () => {
  ...
  t.send("server_start");
  t.send("tool_call", { tool: "get_shop" });
...
test("server_start carries clientInfo from the MCP handshake", async () => {
  ...
  assert.equal(start.body.app, "mcp-shopify-admin");
});
