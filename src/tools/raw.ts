    ...
    "graphql_request",
    {
      ...
      inputSchema: {
        ...
        operationName: z
          ...
          .optional()
          .describe(
            "Имя операции — обязательно, если документ содержит больше одной; без него сервер GraphQL не знает, какую выполнять.",
          ),
      },
    },
    async ({ query, variables, operationName }) => {
      try {
        return ok(await client.request(query, variables, operationName));
      } catch (e) {
        return fail(e);
      }
    },
