...
- **Честные HTTP 200.** Мутации Shopify возвращают 200 даже при провале; сервер читает `userErrors` и превращает отказ в понятную ошибку. Ответ 200 без объекта `data` — пустой, оборванный или вовсе не GraphQL — тоже становится ошибкой, а не пустым результатом.
...
| Создание товара или промокода | Создаёт новую запись; созданный товар не публикуется ни в одном канале продаж | Необратимо создаёт объект |
...
Создание заказов, выполнение отгрузок, изменение клиентов, публикация товаров в каналах продаж и удаление объектов не входят в набор инструментов — при необходимости это делается через `graphql_request` с явным подтверждением.
...
| `SHOPIFY_API_BASE` | Нет | Полный URL GraphQL-эндпоинта по схеме `http` или `https` вместо домена и версии — например локальный мок. |
